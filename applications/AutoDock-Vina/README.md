...
```bash
cd build_docker/
docker build -t docker_vina:latest --build-arg FLAVOR=all .
cd ..
```
* For machine with a proxy:
```bash
cd build_docker/
docker build --build-arg http_proxy=<http_proxy> --build-arg https_proxy=<https_proxy> --build-arg no_proxy=<no_proxy_ip> FLAVOR=all -t docker_vina .
cd ..
```
...
```bash                                                                                                                         
docker run -it -v $INPUT_VINA:/input -v $OUTPUT_VINA:/output docker_vina:latest vina --receptor /input/protein.pdbqt --ligand /input/rand-1.pdbqt --out /output/rand-1_out.pdbqt --center_x 16.459 --center_y -19.946 --center_z -5.850 --size_x 18 --size_y 18 --size_z 18 --seed 1234 --exhaustiveness 64
```
...
Refer to [here](https://github.com/IntelLabs/Open-Omics-Acceleration-Framework/tree/main/applications/AutoDock-Vina/microservice/README.md) for setup instructions and API usage details.
...
This allows you to scale Autodock-vina tasks easily across cloud infrastructure. Refer to [here](https://github.com/IntelLabs/Open-Omics-Acceleration-Framework/tree/main/applications/AutoDock-Vina/nextflow/README.md)
...
Read more about the multiprocess tool [here](https://github.com/IntelLabs/Open-Omics-Acceleration-Framework/tree/main/applications/AutoDock-Vina/multiprocess/README.md)
