...
```bash
cd ../build_docker/
docker build -t autodock_64wi:latest .
cd ../nextflow/
```
...
```bash
nextflow run main.nf   -profile awsbatch_single_node   --protein_list protein_list.txt   --input_dir inputs   --out_dir s3://<bucket-name>/autodock-outputs   -ansi-log false
```
...
`s3://<bucket-name>/autodock-outputs/<run_id>/`
...
```bash
s3://<bucket-name>/autodock-outputs/20250118_153210_curious-hopper/
├── 10gs_job0/
├── 1a30_job1/
├── 1gpk_job2/
├── 1h23_job3/
...
## Docker Image
* Image: `docker.io/<user-name>/autodock_64wi:latest`
