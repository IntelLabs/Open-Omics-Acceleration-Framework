...
mkdir -p /output/output_$timestamp
...
gmx pdb2gmx -f prot_clean.pdb -o prot_pros.gro -water spce -ff amber99sb -ignh || exit_on_error "pdb2gmx failed"
...
gmx grompp -f mdtut_ions.mdp -c prot_solv.gro -p topol.top -o ions.tpr -maxwarn 2 || exit_on_error "grompp for ions failed"

echo "Adding ions..."
echo "13" | gmx genion -s ions.tpr -o prot_solv_ions.gro -p topol.top -pname NA -nname CL -neutral || exit_on_error "genion failed"

echo "Preparing energy minimization..."
gmx grompp -f mdtut_minim.mdp -c prot_solv_ions.gro -p topol.top -o em.tpr -maxwarn 1 || exit_on_error "grompp for minimization failed"
...
while [ $current_ntmpi -ge 1 ]; do
    ...
    if [ -f em.log ] && grep -q "Fatal error: There is no domain decomposition" em.log || grep -q "Fatal error:" em.log; then
        echo "Domain decomposition failed for ntmpi=$current_ntmpi. Reducing MPI ranks..."
        grep "Fatal error:" em.log
        rm -f em.log
...
gmx grompp -f mdtut_nvt.mdp -c em.gro -r em.gro -p topol.top -o nvt.tpr || exit_on_error "grompp for NVT failed"
...
gmx grompp -f mdtut_npt.mdp -c nvt.gro -r nvt.gro -t nvt.cpt -p topol.top -o npt.tpr || exit_on_error "grompp for NPT failed"
...
gmx grompp -f mdtut_md.mdp -c npt.gro -t npt.cpt -p topol.top -o md01.tpr || exit_on_error "grompp for MD failed"
...
mv /input/md01* /output/output_$timestamp/ 2>/dev/null || echo "Warning: No md01* files found."

echo "Pipeline execution completed successfully. Results saved in /output/output_$timestamp"
