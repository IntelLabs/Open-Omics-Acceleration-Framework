...
workDir = 's3://srilekha-new-s3/work-gromacs'
...
Resulting S3 layout:
```bash
s3://srilekha-new-s3/gromacs-outputs/
├── 1abc/
├── 2xyz/
├── 3def/
```
...
```bash
withName: RUN_GROMACS {
  maxForks = 8
}
```

This means:
* Up to **8 proteins run in parallel**
* Each protein uses its own Batch job and container
* No file or runtime conflicts

## How to Run
```bash
nextflow run main.nf -profile awsbatch_single_node --pdb_list "$PWD/pdb_list.txt" --pdb_dir  "$PWD/pdbs" --custom_script "$PWD/run_commands.sh" --out_dir   s3://srilekha-new-s3/gromacs-outputs
```
