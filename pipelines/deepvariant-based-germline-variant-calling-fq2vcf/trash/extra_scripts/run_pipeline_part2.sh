...
mpiexec -bootstrap ssh -bind-to $BINDING -map-by $BINDING --hostfile hostfile -n $N -ppn $PPN python -u test_deep.py --inputdir $INDIR --output  $OUTDIR $TEMPDIR --refdir $REFDIR --index $REF --cpus $CPUS --threads $THREADS --shards $SHARDS   2>&1 | tee ${OUTDIR}/log.txt

#/opt/deepvariant/bin/run_deepvariant --model_type=WGS --ref=/refdir/'+ifile+' --reads=/tempdir/aln'+binstr+'.bam --output_vcf=/output/output.vcf.gz --intermediate_results_dir /tempdir/intermediate_results_dir'+binstr+' --num_shards='+nproc+' --dry_run=false --regions "'+bin_region[i*nranks+rank]+'"'
