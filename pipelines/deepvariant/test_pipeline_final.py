...
def main(argv):
    ...
    for i in range(bins_per_rank):
        ...
        command='mkdir -p '+output+binstr+'; podman run --storage-opt ignore_chown_errors=true -v '+folder+':"/input" -v '+refdir+':"/refdir" -v '+output+'/'+binstr+':"/output" -v '+tempdir+':"/tempdir" localhost/deepvariant:latest python /opt/deepvariant/bazel-out/run_deepvariant.py --model_type=WGS --ref=/refdir/'+ifile+' --reads=/tempdir/aln'+binstr+'.bam --output_vcf=/output/output.vcf.gz --intermediate_results_dir /tempdir/intermediate_results_dir'+binstr+' --num_shards='+nproc+' --dry_run=false --regions "'+bin_region[i*nranks+rank]+'" --pcl_opt'
