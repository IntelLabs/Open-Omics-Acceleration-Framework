...
def get_bam(filename,output,n_shards):
    #shutil.copyfile(filename, filename+"_1")
    # Open the input file in read mode
    with open(filename, 'r') as input_file:
        # Open the output file in write mode
        with open(filename+'_1.txt', 'w') as output_file:
                ...
                if len(processed_line)>0:
                    ...
                    if count < n_shards:
                        ...
                        for i in range(len(lines)):
                            lines[i]="/tempdir/aln"+str(lines[i])+".bam"
                        lines = ','.join(lines)
            ...
            lines = ','.join(lines)
            output_file.write(lines+ '\n')
    shutil.copyfile(filename+'_1.txt', output+"/bam_list")

def update_file_paths(filename):
    shutil.copyfile(filename, filename+"_1")
    # Open the input file in read mode
    with open(filename, 'r') as input_file:
        # Open the output file in write mode
        with open(filename+'_1.txt', 'w') as output_file:
            ...
            for line in input_file:
                ...
                for i in range(len(processed_line)):
                    data = processed_line[i].split("/")[-2]
                    processed_line[i]=os.path.join("/tempdir",data,os.path.basename(processed_line[i]))
                processed_line = ' '.join(processed_line)
                # Write the processed line to the output file
                output_file.write(processed_line+ '\n')
    shutil.copyfile(filename+'_1.txt', filename)

def split_file(input_file,num_parts,folders,path):
    with open(os.path.join(path,input_file), 'r') as f:
        lines = f.readlines()
    part_size = math.ceil(len(lines) / float(num_parts))
    #print(len(lines),float(num_parts),part_size,float(len(lines)) / float(num_parts))
    for i in range(num_parts):
        start = i * part_size
        end = start + part_size if i < num_parts - 1 else len(lines)
        can_file=os.path.join(path,folders[i],input_file)
        #print(can_file)
...
def pragzip_reader_real( comm, cpus, fname, outpipe, outdir, last=False ):
    ...
    if (rank==0): #and not last) or (rank==nranks-1 and last):  # create index
        ...
        f = pz.open( fname, parallelization=maxcpus )
...
ncpus = 8
binrounding = 25000 #as per pacbio
keep=False
...
def main(argv):
    ...
    begin1 = time.time()
    a=run(f'{BINDIR}/applications/mm2-fast/minimap2 -ax map-hifi -t '+cpus+' '+os.path.join(refdir,ifile)+' '+fn1+' > '+fn3 +' 2>'+output+'performance'+str(rank)+'.txt',capture_output=True, shell=True)
    ...
    if rank==0:
        ...
        threads = []
        values=["candidate_file_list","offset_list","counter_file_list","max_candidates"]
        for value in values:
            print("value",value)
        ...
        print("Strategy time:",time.time()-strategy_start)
    comm.barrier()
    deepstart=time.time()
    file = open(output+'/bam_list', 'r')
